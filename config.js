/* Set the following variables in local storage to test locally:
localStorage.setItem('testingResourceAnalysisWithLocalFiles', true);
localStorage.setItem('testingResourceAnalysisWithProxy', true);

Using the proxy requires to be using one. VSCode has a plugin called "Live Server" that can be used to serve the files.
Configure it from 'preferences' > 'settings' 
"liveServer.settings.proxy": {
  "enable": true,
  "baseUri": "/proxy",
  "proxyUri": "http://localhost/ITM.API"
}
*/
